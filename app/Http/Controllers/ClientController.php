...
class ClientController extends Controller
{
        ...
        if (Setting::where('setting_key', 'allow_self_registration')->first()->setting_value == 1) {
            ...
            if ($validator->fails()) {
                ...
                if ($borrower->active == 1) {
                    ...
                    return redirect('client')->with('success', trans('general.successfully_registered_logged_in'));
                }
    ...
    public function storeApplication(Request $request)
    {
        ...
        $application->save();
        return redirect('client/application/' . $application->id . '/show')->with("success" , trans_choice('general.successfully_saved', 1));
    }
    ...
    public function storeGuarantor(Request $request, $loan_application)
    {
        ...
        return redirect('client/application/' . $loan_application->id . '/show')
          ->with("warning",trans_choice('general.successfully_saved', 1));
    }
    ...
    public function acceptGuarantor(Request $request, $id)
    {
        ...
        if ($guarantor->guarantor_id != $request->session()->get('uid')) {
            return redirect()->back()->with("warning" , trans_choice('general.identity_error', 1) );
        }
        $guarantor->status = "accepted";
        $guarantor->accepted_amount = $request->amount;
        $guarantor->save();
        return redirect()->back()->with("success" , trans_choice('general.successfully_saved', 1) );
    }
    ...
    public function showSaving(Request $request, $saving)
    {
        ...
        if (empty(Saving::where('borrower_id', $borrower->id)->first())) {
            return redirect()->back()->with("warning" , trans_choice('general.no_saving_account', 1));
...
// 3. execute and fetch the resulting HTML output
        ...
        if ($output) {
            ...
            //print_r($result);
            if ($result['status'] == 'Ok') {
                return redirect($result['browserurl'])->with("success", "Successful");
            } else {
                 return redirect()->back()->with("warning" , "There was an error processing your request. Please try again");
            }
        } else {
             return redirect()->back()->with("warning" , "There was an error processing your request. Please try again" . curl_error($ch));
        }
    }
    ...
    public function paynowReturnSaving(Request $request, $saving)
    {
        ...
        if ($status == "Paid" || $status = "Awaiting Delivery" || $status = "Delivered") {
             return redirect()->back()->with("warning" ,  trans("general.payment_success"));
            return redirect('client/saving/' . $saving->id . '/show');
        } else {
            //payment was unsuccessful
             return redirect()->back()->with("warning" , "There was an error processing your payment");
            return redirect('client/saving/' . $saving->id . '/show');
        }
    ...
    public function paypalDoneSaving(Request $request, $saving)
    {
        // Thank the user for the purchase
         return redirect('client/saving/' . $saving->id . '/show')->with("warning" ,  trans('general.payment_success'));
...
// 3. execute and fetch the resulting HTML output
        ...
        if ($output) {
            ...
            //print_r($result);
            if ($result['status'] == 'Ok') {
                return redirect($result['browserurl'])->with("warning" , "Successful");
            } else {
                 return redirect()->back()->with("warning" , "There was an error processing your request. Please try again");              
            }
        } else {
...
//stripe payments
    ...
    public function paypalCancel(Request $request, $loan)
    {
        // Curse and humiliate the user for cancelling this most sacred payment (yours)
         return redirect()->back()->with("warning" , trans('general.payment_cancel'));
        return redirect('client/loan/' . $loan->id . '/show');
    }
...
// PAYMENT INVALID & INVESTIGATE MANUALY!
    ...
    public function mpesa(Request $request, $loan)
    {
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS, $data_string);
        $curl_response = curl_exec($curl);
         return redirect()->back()->with("warning" ,  trans_choice("general.mpesa_awaiting_payment", 1));
        return redirect('client/loan/' . $loan->id . '/show');
    }
