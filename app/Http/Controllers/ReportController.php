...
class ReportController extends Controller
{
    ...
    public function trial_balance_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            Excel::store(trans_choice('general.trial_balance', 1) . ' : ' . $request->end_date,
    ...
    public function income_statement_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            Excel::store(trans_choice('general.income', 1) . ' ' . trans_choice('general.statement',
                    1) . ' : ' . $request->end_date,
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function balance_sheet_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            Excel::store(trans_choice('general.balance', 1) . ' ' . trans_choice('general.sheet',
                    1) . ' : ' . $request->start_date,
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function missed_repayments(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        return view('loan_report.missed_repayments',
            compact('start_date',
                'end_date', 'due_items', 'paid_items'));
    }

    public function missed_repayments_pdf(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        $due_items = GeneralHelper::loans_due_items($start_date, $end_date);
        $paid_items = GeneralHelper::loans_paid_items($start_date, $end_date);
        $pdf = PDF::loadView('loan_report.missed_repayments_pdf', compact('start_date',
            'end_date', 'due_items', 'paid_items'));
        return $pdf->download(trans_choice('general.expected', 1) . ' ' . trans_choice('general.repayment',
                2) . ".pdf");
    }

    public function missed_repayments_excel(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        Excel::store(trans_choice('general.expected', 1) . ' ' . trans_choice('general.repayment',
                2),
            function ($excel) use ($data) {
                $excel->sheet('Sheet', function ($sheet) use ($data) {
                    $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function missed_repayments_csv(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
    ...
    public function over_due_loans(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        return view('loan_report.over_due_loans',
            compact('start_date',
                'end_date', 'data'));
    }

    public function over_due_loans_pdf(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        if (!empty($start_date)) {
            $data = LoanTransaction::where('transaction_type',
                ...
                session('branch_id'))->whereBetween('date',
                [$start_date, $end_date])->get();
            $pdf = PDF::loadView('loan_report.over_due_loans_pdf', compact('start_date',
                'end_date', 'data'));
    ...
    public function over_due_loans_excel(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        if (!empty($start_date)) {
            ...
            Excel::store(trans_choice('general.repayment', 2) . ' ' . trans_choice('general.report',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function over_due_loans_csv(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
    ...
    public function collection_sheet(Request $request)
    {
        ...
        return view('loan_report.collection_sheet',
            compact('start_date', 'end_date', 'users', 'user_id', 'data'));
    }
    ...
    public function collection_sheet_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            Excel::store(trans_choice('general.collection', 1) . ' ' . trans_choice('general.sheet',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function arrears_report_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            ...
            Excel::store(trans_choice('general.arrears', 1) . ' ' . trans_choice('general.report',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function past_maturity_loans(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        if (!empty($start_date)) {
            ...
            if ($request->loan_product_id == "-1") {
                $data = Loan::where('maturity_date' , '<', date("Y-m-d"))->where('branch_id',
                    session('branch_id'))->whereBetween('release_date',
                    [$start_date, $end_date])->get();
            } else {
                $data = Loan::where('loan_product_id', $request->loan_product_id)->where('branch_id',
                    session('branch_id'))->where('maturity_date' , '<', date("Y-m-d"))->whereBetween('release_date',
                    [$start_date, $end_date])->get();
            }
        }

        return view('loan_report.past_maturity_loans',
            compact('start_date',
                'end_date', 'data', 'user_id', 'loan_product_id', 'users', 'loan_products'));
    }

    public function past_maturity_loans_excel(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
        ...
        if (!empty($end_date)) {
            if ($request->loan_product_id == "-1") {
                $ldata = Loan::where('maturity_date' , '<', date("Y-m-d"))->where('branch_id',
                    session('branch_id'))->whereBetween('release_date',
                    [$start_date, $end_date])->get();
            } else {
            ...
            Excel::store(trans_choice('general.disbursed', 1) . ' ' . trans_choice('general.loan',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function past_maturity_loans_csv(Request $request)
    {
        if (!Sentinel::hasAccess('reports')) {
            Flash::warning("Permission Denied");
            return redirect('/');
    ...
    public function provisioning_excel(Request $request)
    {
        ...
        if (!empty($end_date)) {
            ...
            Excel::store(trans_choice('general.provisioning', 1) . ' ' . trans_choice('general.report',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
        ...
        if (!empty($end_date)) {
            ...
            foreach (LoanProduct::get() as $key) {
                ...
                foreach (Loan::where('loan_product_id', $key->id)->where('branch_id',
                    ...
                    $past_maturity_loans = $past_maturity_loans + 1;
                    $loan_due_items = \App\Helpers\GeneralHelper::loan_due_items($key->id);
                    $loan_paid_items = \App\Helpers\GeneralHelper::loan_paid_items($key->id);
                ...
                $total_past_maturity_loans = $total_past_maturity_loans + $past_maturity_loans;
            ...
            Excel::store(trans_choice('general.product', 2) . ' ' . trans_choice('general.summary',
                    1),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
    ...
    public function general_report(Request $request)
    {
        ...
        $monthly_past_maturity_loans_data = [];
        $loop_date = date_format(date_sub(date_create($date),
            date_interval_create_from_date_string('1 years')),
            'Y-m-d');
        for ($i = 1; $i < 14; $i++) {
            ...
            array_push($monthly_past_maturity_loans_data, array(
                'month' => date_format(date_create($loop_date),
                    'M' . ' ' . $d[0]),
                'value' => $principal,
            ));
        ...
        $monthly_past_maturity_loans_data = json_encode($monthly_past_maturity_loans_data);
        return view('company_report.general_report',
            compact('loan_product_data', 'monthly_net_income_data', 'monthly_borrower_data', 'monthly_repayments_data',
                'monthly_actual_expected_data', 'monthly_past_maturity_loans_data','start_date','end_date'));
    }
    ...
    public function savings_transactions_excel(Request $request)
    {
        ...
        if (!empty($start_date)) {
            ...
            Excel::store(trans_choice('general.saving', 2) . ' ' . trans_choice('general.transaction',
                    2),
                function ($excel) use ($data) {
                    $excel->sheet('Sheet', function ($sheet) use ($data) {
                        $sheet->fromArray($data, null, 'A1', false, false);
