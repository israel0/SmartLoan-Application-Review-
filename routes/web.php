    ...
    function () {
        ...
        for ($i = 1; $i < 14; $i++) {
            ...
            foreach (\App\Models\Loan::select("loan_schedules.principal", "loan_schedules.interest", "loan_schedules.penalty",
                ...
                $d[1])->get() as $key) {
                $missed = $missed + $key->interest + $key->penalty + $key->fees + $key->principal;
                $principal = $principal + $key->principal;

            }
            array_push($monthly_actual_missed_data, array(
        ...
        $monthly_actual_missed_data = json_encode($monthly_actual_missed_data);
        ...
        $release = ReleaseNote::orderBy("id", "desc")->first();

        return view('dashboard', compact('monthly_actual_missed_data', 'monthly_past_maturity_loans_data', 'loan_statuses', 'release'));
    }
])->name('dashboard');
...
Route::group(['prefix' => 'setting'], function () {
    ...
    Route::post('update-sms' , 'SettingController@update_sms');
...
Route::group(['prefix' => 'report'], function () {
    ...
    Route::group(['prefix' => 'loan_report'], function () {
        Route::any('missed_repayments', 'ReportController@missed_repayments');
        Route::any('missed_repayments/pdf', 'ReportController@missed_repayments_pdf');
        Route::any('missed_repayments/excel', 'ReportController@missed_repayments_excel');
        Route::any('missed_repayments/csv', 'ReportController@missed_repayments_csv');
        Route::any('over_due_loans', 'ReportController@over_due_loans');
        Route::any('over_due_loans/pdf', 'ReportController@over_due_loans_pdf');
        Route::any('over_due_loans/excel', 'ReportController@over_due_loans_excel');
        Route::any('over_due_loans/csv', 'ReportController@over_due_loans_csv');
        ...
        Route::any('past_maturity_loans', 'ReportController@past_maturity_loans');
        Route::any('past_maturity_loans/pdf', 'ReportController@past_maturity_loans_pdf');
        Route::any('past_maturity_loans/excel', 'ReportController@past_maturity_loans_excel');
        Route::any('past_maturity_loans/csv', 'ReportController@past_maturity_loans_csv');
        Route::any('cash_flow', 'ReportController@cash_flow');
...
Route::group(['prefix' => 'communication'], function () {
    ...
    Route::get('getbalance', 'CommunicationController@getbalance')->name('getbalance');
...
Route::post("open-ticket" , "OpenticketController@store")->name("open-ticket");
Route::get('openticket/data', 'OpenticketController@index')->name("view-ticket");

//Release Note
Route::get("release-note", "ReleaseNoteController@index")->name("release-note");
Route::post("store-release-note", "ReleaseNoteController@store")->name("store-release-note");
Route::post("update-release-note/{id}", "ReleaseNoteController@update")->name("update-release-note");
Route::get("delete-release-note", "ReleaseNoteController@delete")->name("delete-release-note");
Route::post("change-release-note-status", "ReleaseNoteController@changeStatus");